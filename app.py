...
soil_model_error = plant_model_error = None
soil_model = plant_model = None
...
def explain_prediction(label: str, category: str) -> str:
    ...
    prompt = (
        ...
        "Give practical steps, precautions, and guidance in English and Arabic.\n\n"
        "### English Explanation\n- Use bullet points for actions.\n\n"
        "### Arabic Explanation (الفهم بالعربية)\n- اشرح باللغة العربية السهلة مع خطوات واضحة.\n"
    )
    try:
...
def show_user_page():
    ...
    with left_col:
        ...
        input_method = st.radio("Choose input:", ["Upload Image", "Use Camera"], key="input_method")
        img = None
        if input_method == "Upload Image":
            uploaded = st.file_uploader("Upload a soil or plant image", type=["jpg","jpeg","png"], key="up_img")
            if uploaded:
                img = Image.open(uploaded).convert("RGB")
        else:
            cam_img = st.camera_input("Take a live photo", key="cam_img")
            if cam_img:
    ...
    with right_col:
        ...
        if img:
            ...
            st.markdown('<div class="warning-box">No image yet.</div>', unsafe_allow_html=True)
        task_type = st.radio("Select Task", ["Soil Moisture", "Plant Disease"], horizontal=True, key="task_type")
        st.markdown('</div>', unsafe_allow_html=True)
